...
//   // State for pagination, search query, and filtered products
...
//           <p className="text-lg font-bold text-red-500 absolute bottom-4 right-4 py-8">{`$${product.price.toFixed(2)}`}</p>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {currentProducts.map(product => (
            <div key={product.id} className="bg-white p-4 shadow-md rounded-2xl border-2 border-black relative">
              <img src={product.thumbnail} alt={product.title} className="w-full h-48 object-cover mb-2 rounded" />
              <p className="text-lg font-bold text-red-500 absolute bottom-4 right-4">{`$${product.price.toFixed(2)}`}</p>
              <h3 className="text-lg font-semibold">{product.title}</h3>
            </div>
          ))}
        </div>
